...
public class GprsTrafficImpl implements IGprsTraffic {
	...
	public GprsTrafficImpl(Context context) {
	...
	public String getThisMonthTrafficOfGprs() {
		...
		sql = "select sum(gprs) as gprs from traffic where date>=? and date<?";
	...
	public String getTodayTrafficOfGprs() {
		...
		sql = "select gprs from traffic where date=?";
	...
	public String getBeforeGprsTraffic(String today) {
		...
		SQLiteDatabase db = mDBService.getReadableDatabase();
		sql = "select sum(gprs) as gprs from traffic where date<?";
		Cursor mCursor = db.rawQuery(sql, new String[]{today});
		if (mCursor != null && mCursor.getCount() > 0) {
			if (mCursor.moveToFirst()) {
				do {
	...
	public String getMaxDate() {
		String maxDate = null;
		SQLiteDatabase db = mDBService.getReadableDatabase();
		sql = "select max(date) as date from traffic";
	...
	public List<String> getEveryDayTraffic() {
		...
		SQLiteDatabase db = mDBService.getReadableDatabase();
		sql = "select gprs from traffic where date>=? and date <=?";
	...
	public String getGprs1Traffic(String powerOnDate, String today) {
		String gprs1 = "0";
		SQLiteDatabase db = mDBService.getReadableDatabase();
		String sql = "select sum(gprs1) as gprs1 from traffic where date>='" + powerOnDate
					+ "' and date<'" + today + "'";
		Cursor mCursor = db.rawQuery(sql, null);
		if (mCursor != null && mCursor.getCount() > 0) {
			if (mCursor.moveToFirst()) {
