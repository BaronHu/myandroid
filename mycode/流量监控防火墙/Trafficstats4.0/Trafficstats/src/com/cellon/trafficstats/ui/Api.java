...
public final class Api {
     ...
     * @param msg message
     */
	public static void alert(Context ctx, int msgId) {
    	if (ctx != null) {
...
//			"";
			"# Try to find iptables\n" +
			"if " + dir + "/iptables --version >/dev/null 2>/dev/null ; then\n" +
	...
	private static boolean applyIptablesRulesImpl(Context ctx, List<Integer> uidsWifi, List<Integer> uids3g, boolean showErrors) {
		...
		try {
			...
			if (showErrors && code != 0) {
				...
				alert(ctx, R.string.error_applying_iptables_rules);
			} else {
				return true;
			}
		} catch (Exception e) {
			if (showErrors) alert(ctx, R.string.error_refreshing_iptables);
		}
		return false;
    }
    /**
	...
	public static boolean purgeIptables(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (code == -1) {
				if (showErrors) alert(ctx, R.string.error_purging_iptables);
				return false;
			}
			return true;
		} catch (Exception e) {
	...
	public static void showIptablesRules(Context ctx) {
		try {
			...
			runScriptAsRoot(ctx, scriptHeader(ctx) +
								 "$ECHO $IPTABLES\n" +
								 "$IPTABLES -L -v\n", res);
//			alert(ctx, res);//baron.hu
		} catch (Exception e) {
//			alert(ctx, "error: " + e);//baron.hu
		}
	}
	...
	public static boolean clearLog(Context ctx) {
		try {
			...
			if (code != 0) {
				alert(ctx, R.string.error_clear_log);
				return false;
			}
			return true;
		} catch (Exception e) {
			alert(ctx, R.string.error);
		}
		return false;
	}
	/**
	...
	public static void showLog(Context ctx) {
		try {
			...
			if (code != 0) {
				...
				alert(ctx, R.string.error_show_log);
				return;
			}
	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			Drawable appIcon = null;//baron.hu
			DroidApp app = null;
			for (final ApplicationInfo apinfo : installed) {
				...
				appIcon = pkgmanager.getApplicationIcon(apinfo);//baron.hu
				if (name.length() == 0) {
...
//			alert(ctx, R.string.no_root_permission);//baron.hu
		}
		return false;
	}
    /**
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (changed) {
//				Toast.makeText(ctx, R.string.toast_bin_installed, Toast.LENGTH_LONG).show();//baron.hu
			}
		} catch (Exception e) {
			if (showErrors) alert(ctx, R.string.error_installing_binary_files);
			return false;
		}
		return true;
	}
...
//        		if (uid > 0) s.append(uid + ": ");//baron.hu
...
//        		}
	...
	private static final class ScriptRunner extends Thread {
		...
		//baron.hu
		private final String PATH = "/system/bin/iptables";
		...
		public void run() {
			try {
				file.createNewFile();
				final String abspath = file.getAbsolutePath();
				Log.d("baron.hu", "abspath=" + abspath);
...
//					exec = Runtime.getRuntime().exec("su -c "+abspath);
				...
				Log.d("baron.hu", "this.exitcode=" + this.exitcode);
			} catch (InterruptedException ex) {
				ex.printStackTrace();
	 ...
	 * @param context
	 * @return list list of every application icon
	 * */
	public static List<DroidApp> getDrawable(Context context) {
		final PackageManager pkgmanager = context.getPackageManager();
		List<ApplicationInfo> apps = pkgmanager.getInstalledApplications(0);
		List<DroidApp> icons = new ArrayList<DroidApp>();
		try {
			for (int i = 0; i < apps.size(); i++) {
				ApplicationInfo ainfo = apps.get(i);  
				DroidApp app = new DroidApp();
				app.appIcon = pkgmanager.getApplicationIcon(ainfo);
				icons.add(app);
			}
		} catch (Exception e) { 
			e.printStackTrace();
	...
	/**
     * baron.hu
     * get the network status of WIFI
     * 
     * */
